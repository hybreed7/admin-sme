...
function SetNewApproval() {
  ...
  const fetchModule = async () => {
    setIsLoading(true);
    try {
      const response = await axios.get('https://api-sme.promixaccounting.com/api/v1/module', { headers });
...
const createApproval = async () => {
  ...
  try {
    const response = await axios.post(
      'https://api-sme.promixaccounting.com/api/v1/approval_level/create_app',
      {
