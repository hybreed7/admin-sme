...
const navigate = useNavigate();
  ...
  const createUnit = async () => {
    ...
    try {
      const response = await axios.post(
        'https://api-sme.promixaccounting.com/api/v1/units/create',
        {
          name: name,
        },
        { headers }
  ...
  const handleEyeClick = (id) => {
    const fountUnits = tableData.find(item => item.id === id);
    const unitId = fountUnits.id;
    setSelectedUnit(unitId);
 
    
    const { name } = fountUnits;
    
    setName1(name || '');
    // setDescription1(description || '');
    // setglCode1(glCode || '');
    // setReOrderLevel1(reOderLevel || '');
    // setUnit1(unit || '');
  ...
  const handleTrashClick = async (id) => {
    try {
      const response = await axios.get(`https://api-sme.promixaccounting.com/api/v1/stocks/delete_stock?id=${id}`, { headers });
      fetchUnits();
      Swal.fire({
        icon: 'success',
        title: 'Success',
  ...
  const editStock = async () => {
    ...
    try {
      const response = await axios.post(
        'https://api-sme.promixaccounting.com/api/v1/stocks/update_stock',
        {
            name: name1,
            // description: description1,
            // unit_price: price1,
            // unit_of_measurement: measurement1,
            // quantity: quantity1
        },
        { headers }
      );
              ...
              <div className="col-sm-8 header-title p-0">
                <div className="media">
                  ...
                  <div className="media-body">
                    <h1 className="font-weight-bold">My Units</h1>
                    <small>Create and view your units...</small>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="body-content">
              ...
              <Modal show={show} onHide={handleClose} animation={false}>
                      ...
                      <Modal.Footer>
                        ...
                        <Button variant="success" onClick={createUnit}>
                    {loading ? (
                      <>
                      <Spinner  size='sm' /> 
                      <span style={{ marginLeft: '5px' }}>Creating unit, Please wait...</span>
    </>
  ) : (
                "Create Unit"
                      )}
                              ...
                              <tbody style={{ whiteSpace: 'nowrap' }}>
                                {displayedData.map((item, index) => (
                                  <tr key={index}>
                                    <td>{index + 1}</td>
                                    <td>{item.name}</td>
                                    {/* <td>{formatDate(item.creatd_at)}</td> */}
