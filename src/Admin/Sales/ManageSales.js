...
import classes from './ManageSales.module.css';
...
const navigate = useNavigate();
  ...
  const fetchSales = async () => {
    setIsLoading(true);
    try {
      const response = await axios.get('https://api-sme.promixaccounting.com/api/v1/sales/transactions', { headers });
  ...
  const filteredData = tableData.filter(item => item.receipt_number.toLowerCase().includes(searchTerm.toLowerCase()));
              ...
              <div className="col-sm-8 header-title p-0">
                <div className="media">
                  ...
                  <div className="media-body">
                    <h1 className="font-weight-bold">Sales</h1>
                    <small>Create and view your sales...</small>
                  </div>
                </div>
              </div>
            </div>
                              ...
                              <tbody style={{ whiteSpace: 'nowrap' }}>
                                {displayedData.map((item, index) => (
                                  <tr key={index}>
                                    ...
                                    <td style={{textAlign: "left"}}>{item.item?.name}</td>
                                    <td>{item.quantity}</td>
                                    <td>{parseFloat(item.price).toLocaleString('en-US', {
                                    ...
                                    <td style={{textAlign: "left"}}>{item.receipt_number}</td>
                                    <td style={{textAlign: "left"}}>{item.users?.name}</td>
                                    <td>
